...
void Mod_LoadTexinfo(lump_t* l)
{
	...
	for (i = 0; i < count; i++, in++, out++)
	{
		...
		out->image = DX11_FindImage(name, it_wall);
		if (!out->image)
		{
...
void Mod_LoadAliasModel(model_t* mod, void* buffer)
{
	...
	{
		mod->skins[i] = DX11_FindImage((char*)pheader + pheader->ofs_skins + i * MAX_SKINNAME
			, it_skin);
	}
...
void Mod_LoadSpriteModel(model_t* mod, void* buffer)
{
	...
	for (i = 0; i < sprout->numframes; i++)
	{
		...
		mod->skins[i] = DX11_FindImage(sprout->frames[i].name, it_sprite);
	}
...
struct model_s* R_RegisterModel(char* name)
{
	...
	if (mod)
	{
		...
		else if (mod->type == mod_alias)
		{
			...
			for (i = 0; i < pheader->num_skins; i++)
				mod->skins[i] = DX11_FindImage((char*)pheader + pheader->ofs_skins + i * MAX_SKINNAME, it_skin);
			//PGM
